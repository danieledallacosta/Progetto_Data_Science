...
df_x <- data.frame(indice, AllS[, 1], as.numeric(AllS[, 2]), AllS[, 3])

names(df_x) <- c("Indice", "Specialita", "MediaP", "Stagione")


p <- ggplot(df_x, aes(x = Specialita, y = MediaP, fill = Specialita)) +
  geom_bar(stat = "identity") +
  labs(title = "Stagione: {closest_state}") +
  ylab("Media") +
  xlab("Nome Disciplina") +
  geom_text(aes(label = MediaP, y = MediaP),
            position = position_dodge(0.4), vjust = -0.5, size = 5, color = "black") +
  theme_classic() +
...
highchart() %>% 
  hc_chart(object = nVpP, type ="bar") %>%
    ...
    hc_xAxis(categories = nVpP$'Pista',
             style = list(
                fontWeight = "bold")) %>%
    hc_yAxis(categories = as.character(c(0:10))) %>%
